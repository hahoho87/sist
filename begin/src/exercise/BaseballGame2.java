...
public class BaseballGame2 {
	...
	public static void main(String[] args) {
		...
		man = new int[] { 3, 7, 9 }; // 내가 선택한 숫자
		System.out.println("{" + man[0] + ", " + man[1] + ", " + man[2] + "}" + "\n");

		if (man[0] == man[1] || man[0] == man[2] || man[1] == man[2]) {
			System.out.println("중복되지 않는 숫자로 입력해 주세요.");
			return; // break 사용 불가, 메소드 멈추고 싶을 때 return
		} else
		...
		while (playball) {
			...
			for (int i = 0; i < man.length; i++) {
				for (int j = 0; j < man.length; j++) {
					...
					if (man[0] == com[0] && man[1] == com[1] && man[2] == com[2]) // 세개 모두 같으면 중지
					{
						playball = false;
